...
RegisterNetEvent('pis:askid')
AddEventHandler('pis:tcallouts', function()
		ShowNotification("~b~TCallouts was made by Tonaye you can buy @ Discord: Tonyy#3713")
		Wait(500)
		ShowMenuHelp("boop")
		Wait(500)
		ShowMenuHelp("E for help")
		Wait(1000)
		ShowNotification("~r~If you need any help please ask a question or submit a bug on the download thread!")
end)

RegisterNetEvent('pis:askidped')
AddEventHandler('pis:askidped', function()
	local player = GetPlayerPed(-1)
	local playerPos = GetEntityCoords(player)
	local inFrontOfPlayer = GetOffsetFromEntityInWorldCoords (player, 0.0, 5.0, 0.0)
	local cuffingPed = GetPedInDirection( playerPos, inFrontOfPlayer )
	local targetPed = cuffingPed
	local pedPos = GetEntityCoords(targetPed)
	local distanceToVeh = GetDistanceBetweenCoords(GetEntityCoords(player), GetEntityCoords(cuffingPed))
	if distanceToVeh <= 5 then
		if speech == "Normal" then
			local OfficerNormalQuotes = {"Can i see some ID?", "ID, please.", "License or ID?"}
			ShowNotification("~o~Officer:~w~ " .. OfficerNormalQuotes[math.random(#OfficerNormalQuotes)])
		else
			local OfficerAggresiveQuotes = {"Show me your ID.", "Give me your ID.", "Give me your fucking license.", "Show me your info."}
			ShowNotification("~o~Officer:~w~ " .. OfficerAggresiveQuotes[math.random(#OfficerAggresiveQuotes)])
		...
		Wait(2000)
			local Id = {"Sammy Smith", "Adrian Long", "Andy Faith", "Amari Chris", "Delta Alex", "Devin Stevens", "Dallas Zed", "Jake Evens", "Dorian Evens", "Chris Oic", "August Summer", "Aspen Devin", "Aubrey Evans"}
			local IdR = {"Is it because I'm black?","Just take it already...","Uhm.. Sure... Here.","But I've done nothing wrong, sir!", "Pig.", "Why dont you go and fight real crime?", "Yeah, sure.","Okay, here you go.","There.","Take it, just hurry up please.","*Gives ID*","Okay, here you go.","Sure thing!","Alright, no problem.","Yep, there it is."}
			ShowNotification("~b~Ped:~w~ " .. IdR[math.random(#IdR)])
			ShowNotification("~r~Id:~w~ " .. Id[math.random(#Id)])
			ShowNotification("~r~ You can purchase this AskID script on discord Tonyy#3713")
			driverQuestioned = true
	else
		ShowNotification("~r~You need to be closer to the PED!")
	end
end)
