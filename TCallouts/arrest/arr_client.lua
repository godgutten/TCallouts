...
AddEventHandler('pis:arr:pt', function()
    ...
    local pmodels = {"s_m_y_hwaycop_01"}
    local vehicles = {"msp2"}
...
RegisterNetEvent('pis:arr:bu')
AddEventHandler('pis:arr:bu', function()
	local spawnDistance = math.random(spawnDistance, spawnDistance + 25)
	local player = GetPlayerPed(-1)
    local playerPos = GetEntityCoords(player)
    local pmodels = {"s_m_y_hwaycop_01"}
    ...
    RequestModel(driver)
		...
		if DoesEntityExist(targetPed) then
			...
			ShowNotification('~o~Officer: ~w~Dispatch, requesting 10-32 to my 20.')
			...
			ShowNotification('~o~Dispatch: ~w~A 10-32 has been dispatched to your location. ~w~\nETA: ' .. eta)
			enroute = true
			while (enroute) do
			Citizen.Wait(300)
			local distanceToVeh = GetDistanceBetweenCoords(GetEntityCoords(vehicle), GetEntityCoords(targetPed), 1)
				...
				SetVehicleSiren(vehicle, true)
					...
					if GetEntitySpeed(vehicle) < 5 then
						...
						if isopen ~= 0 then
							RemovePedFromGroup(cuffingPed)
							TaskEnterVehicle(targetPed, vehicle, 10000, 1, 2.0, 1, 0)
							ShowNotification("~o~Officer: ~w~Stop where you're at!!")
							...
							if IsPedInVehicle(targetPed, vehicle, false) then
								...
								ShowNotification("~o~Driver:~w~ I've got it from here.")
								enroute = false
								vanBlip = RemoveBlip(vanBlip)
								SetVehicleIndicatorLights(vehicle, 1, false)
								SetVehicleIndicatorLights(vehicle, 2, false)
